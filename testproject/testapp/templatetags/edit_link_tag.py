...
def reverse_edit_link(obj):
    ...
    try:
        ...
        raise VariableIsNotModelInstance
    return urlresolvers.reverse('admin:%s_%s_change' % (app_label, obj_name),
        args=(obj_id,))
...
class EditLinkNode(template.Node):
    ...
    def render(self, context):
        ...
        return '<a href="%s">Edit %s "%s"</a>' \
            % (link, object_name(obj), unicode(obj))
...
def compile_edit_link_tag(parser, token):
    try:
        ...
        raise template.TemplateSyntaxError, "edit_link tag \
